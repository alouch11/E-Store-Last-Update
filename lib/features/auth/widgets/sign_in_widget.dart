...
/*      if(!Provider.of<AuthController>(context, listen: false).isGuestIdExist()){
      ...
      if (Provider
          ...
          .isRemember!) {
        Provider.of<AuthController>(context, listen: false).saveUserEmail(
            email, password);
      } else {
        ...
        if (Provider
          ...
          Provider.of<AuthController>(context, listen: false).sendOtpToEmail(
            ...
            if (value.response?.statusCode == 200) {
              Provider.of<AuthController>(context, listen: false).updateEmail(
                  _emailController!.text.toString());
              Navigator.pushAndRemoveUntil(
                  context, MaterialPageRoute(builder: (_) =>
                  VerificationScreen(
                      temporaryToken, '', _emailController!.text.toString())), (
                  route) => false);
            }
          });
        } else if (Provider
                              ...
                              child: Container(alignment: Alignment.center,
                                  decoration: BoxDecoration(
                                      border: Border.all(color: Theme
                                          .of(context)
                                          .primaryColor
                                          .withOpacity(.75), width: 1.5),
                                      borderRadius: BorderRadius.circular(6)),
                                  child: Icon(
                                      CupertinoIcons.checkmark_alt, size: 15,
                                      color: authProvider.isRemember! ? Theme
                                          .of(context)
                                          .primaryColor
                                          .withOpacity(.75) : Colors
                                          .transparent))));
                    }),
...
/*                InkWell(onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const ForgetPasswordScreen())),
          ...
          Container(margin: const EdgeInsets.only(bottom: 20, top: 30),
            ...
            Center(child: CircularProgressIndicator(
              valueColor: AlwaysStoppedAnimation<Color>(Theme
                  ...
                  .primaryColor,),),) :
            Hero(tag: "onTap",
                child: CustomButton(onTap: loginUser,
          ...
          InkWell(onTap: () async {
            ...
            setState(() {
              List<String> queryParams =barcodeScanRes.toString().split(';');
              _emailController!.text= loginBody.email=queryParams[0] ??'';
              _passwordController!.text= loginBody.password =queryParams[1]??'';

              loginBody.guestId = Provider.of<AuthController>(context, listen: false).getGuestToken() ?? '1';

              Provider.of<AuthController>(context, listen: false).login( loginBody, route);

            });
          },
            child: Padding(padding: const EdgeInsets.all(2),
              child: Container(alignment: Alignment.center, width:MediaQuery.of(Get.context!).size.width/2.5, height: 40,
                ...
                child: Padding(
                  ...
                  child: Row(
                    children: [
                      ...
                      const Icon(
                          Icons.qr_code_scanner_outlined, color: Colors.white, size: 30),
                      const SizedBox(width: 5,),
                      Text(getTranslated('use_qr_code', context)!, style: textBold.copyWith(fontSize: Dimensions.fontSizeLarge,color: Colors.white),),
                    ],
                  ),
                ),
              ),
