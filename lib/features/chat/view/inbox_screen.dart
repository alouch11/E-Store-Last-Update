...
class _InboxScreenState extends State<InboxScreen> {
  ...
  void initState() {
    ...
    isGuestMode = !Provider.of<AuthController>(context, listen: false).isLoggedIn();
      if(!isGuestMode) {
        ...
        Provider.of<ChatProvider>(context, listen: false).getUserList(context, 1, reload: false);
      }
  ...
  Widget build(BuildContext context) {
    return PopScope(canPop: false,
      ...
      child: Scaffold(
        appBar: CustomAppBar(title: getTranslated('chat', context), isBackButtonExist: widget.isBackButtonExist,
        onBackPressed: (){
          if(Navigator.of(context).canPop()){
            Navigator.of(context).pop();
          }else{
              ...
              child: isGuestMode ? const NotLoggedInWidget() :  RefreshIndicator(
                ...
                child: Consumer<ChatProvider>(
                  builder: (context, chatProvider, child) {
                    return chatProvider.userModel != null? (chatProvider.userModel!.user != null && chatProvider.userModel!.user!.isNotEmpty )?
                    ListView.builder(
                      itemCount: chatProvider.userModel?.user?.length,
                      padding: const EdgeInsets.all(0),
                      itemBuilder: (context, index) {
                        return ChatItemWidget(user: chatProvider.userModel!.user![index], chatProvider: chatProvider);
